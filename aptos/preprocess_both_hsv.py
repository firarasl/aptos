...
def convert_to_hsv(img):
    """Convert an image to HSV using OpenCV."""
    if len(img.shape) == 3 and img.shape[2] == 3:  # Check if the image is RGB
        return cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
    return img  # If already HSV, return as is
...
def process_and_save(img_path, save_dir, processor):
    ...
    try:
        ...
        img = convert_to_hsv(img)  # Convert to HSV
...
def preprocess_dataset_1():
    ...
    PROCESS_DIR = Path('/src/rfir/project/aptos2019-blindness-detection/data/preprocessed/hsv')
...
def preprocess_dataset_2():
    ...
    PROCESS_DIR = Path('/src/rfir/project/aptos2019-blindness-detection/data/preprocessed/hsv/diabetic-retinopathy-detection')
    PRO_TRAIN_DIR = PROCESS_DIR / 'train_images'
    PRO_TEST_DIR = PROCESS_DIR / 'test_images'
...
def main():
    print("Preprocessing Dataset Aptos HSV ...")
    preprocess_dataset_1()

    print("\nPreprocessing Dataset DR HSV ...")
